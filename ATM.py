...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        # self.configure(fg_color="#00A3F1")

        self.account_no = ctk.StringVar()
        self.user_pin = ctk.StringVar()

        # The frames will be displayed by pack()
        self.home_screen = ctk.CTkFrame(self, fg_color="#00A3F1")
        self.screen_list = [self.home_screen]
    ...
    def add_homescreen_elements(self, account_no, user_pin):
        ...
        heading = ctk.CTkLabel(self.home_screen, text="Welcome to the ATM", fg_color="purple", text_color="red", font=("Arial", 16, "bold"))
        heading.grid(row=0, column=4, sticky="nsew", columnspan=2)
        
        query = ctk.CTkLabel(self.home_screen, text="How may we help you?", fg_color="yellow", text_color="green", font=("Arial", 16))
        query.grid(row=3, column=4, sticky="nsew", columnspan=2)
        
        bank_label = ctk.CTkLabel(self.home_screen, text="Kasukabe Bank", fg_color="cyan", text_color="violet", font=("Arial", 16, "bold"))
        bank_label.grid(row=2, column=4, sticky="nsew", columnspan=2)
        
        create_button = ctk.CTkButton(self.home_screen, text="Create Account", command=None)
        create_button.grid(row=6, column=5, sticky="nsew")
        
        login_button = ctk.CTkButton(self.home_screen, text="Login", command=self.verify_user)
        login_button.grid(row=6, column=4, sticky="nsew")

        acc_no_entry = ctk.CTkEntry(self.home_screen, placeholder_text="Enter your Account Number", corner_radius=8, textvariable=account_no)
        acc_no_entry.grid(row=4, column=4, sticky="nsew", columnspan=2)

        pin_entry = ctk.CTkEntry(self.home_screen, placeholder_text="Enter your PIN", corner_radius=8, textvariable=user_pin)
        pin_entry.grid(row=5, column=4, sticky="nsew")

        self.error_message = ctk.CTkLabel(self.home_screen, text="Please enter valid credentials", text_color="red", font=("Arial", 16, "bold"))
    ...
    def show_error_message(self):
        self.error_message.grid(row=4, column=6, sticky="nsew", columnspan=2)
